...
| Simple UI 独立底栏候选项 | 通过 | `tests/main_spec.lua` 验证 `bookshelf_open` 注册/注销；不写 `simpleui_bar_tabs`，原“书籍”动作保持不变 |
| 卡片高度与四边圆角边框 | 几何通过 | 缓存图尺寸固定为外框减两倍边框；分类和书籍共用最大标题槽及作者占位，封面统一顶对齐；缓存渲染版本升级为 2 |
| 与 Simple UI、CoverBrowser 同时启用 | 静态兼容通过 | 使用 Simple UI 的 `QA.register/QA.unregister`，没有目标类方法赋值或对方设置键写入；长按注册键互不相同 |
| 禁用恢复 | 通过 | `stopPlugin/onTeardown` 关闭活动 Grid、取消缩略图任务、注销 `bookshelf_open`、移除 `bookshelf_add_to_shelf`、只刷新私有设置 |
