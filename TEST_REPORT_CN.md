...
| 阅读进度与角标底色 | 几何通过 | 读取 2026.07.1 的 `BookList.getBookInfo(path)`；未开始隐藏，完成显示 100%；白/灰底使用黑字，黑底使用白字；扫描线绘制圆角平顶与柔和尖底，中心锚定封面宽度 `75%`、顶部上移 3 个缩放单位；固定文字容器按最宽的 `100%` 计算，保证 `1%`、`52%`、`100%` 与三种底色的角标尺寸和位置一致 |
