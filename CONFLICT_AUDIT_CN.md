...
Bookshelf 没有给 FileChooser、FileManager、CoverBrowser、History、Collections 或 FileSearcher 的类/实例方法赋值；唯一长按注册键为 `bookshelf_add_to_shelf`，禁用时按同一键移除。插件只读全局 `home_dir` 与 `language`，不会写 `start_with`，不会读写 `simpleui_*`、CoverBrowser SQLite 配置或两个插件的文件。启动书架使用自身布尔设置和 `FileManager.registerPostInitCallback`；默认关闭，回调异常由 `pcall` 捕获并仅写日志。
