...
2026-09-15：书架首页改为分类与未分类单书直接混排；新增 Simple UI 独立底栏候选动作和页面识别；加入事务式分类选择、分类返回及阅读后恢复原分类；分类封面增加透视叠层线和灰色数量，作者改为灰色，封面边框加粗并最后绘制；标题栏新增书籍排序入口，支持手动、名称、文件修改时间和最后阅读时间。
